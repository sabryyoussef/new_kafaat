#!/usr/bin/env python3
"""Prepare sabry-test fixture for S5 Playwright (batch + portal students)."""
...
def kw(model, method, *args, **kwargs):
    # Odoo XML-RPC: execute_kw(db, uid, password, model, method, args, kwargs)
    return m.execute_kw(DB, uid, PWD, model, method, list(args), kwargs)
...
batch = kw('op.batch', 'read', [batch_id], fields=[
    'attendance_qr_token', 'attendance_qr_url', 'name',
])[0]
